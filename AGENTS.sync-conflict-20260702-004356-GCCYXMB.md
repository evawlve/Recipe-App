...
| Pattern | Example |
|---------|---------|
| Cache tables | `FdcFood`, `OffFood`, `FdcServing`, `OffServing` |
...
> ⚠️ **Critical**: FDC foods use integer `fdcId`. Open Food Facts (OFF) products use string `barcode`. Never mix these.
...
// Open Food Facts: String ID (barcode)
model OffServing {
