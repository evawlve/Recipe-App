...
// We'll define state and report paths dynamically in main() based on CLI flags
...
async function callGemini(batch: MappingRow[], model: string, baseUrl: string, apiKey: string): Promise<EntryVerdict[]> {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 120_000); // 120s timeout for local ollama
...
async function main() {
    ...
    const skipRows   = Number(args.find(a => a.startsWith('--skip='))?.split('=')[1] ?? '0');
    const limitRows  = Number(args.find(a => a.startsWith('--limit='))?.split('=')[1] ?? '0');
    const provider   = args.find(a => a.startsWith('--provider='))?.split('=')[1] ?? 'openrouter';
    const model      = args.find(a => a.startsWith('--model='))?.split('=')[1] ?? (provider === 'ollama' ? 'qwen2.5:14b' : DEFAULT_MODEL);
    const delayMs    = Number(args.find(a => a.startsWith('--delay-ms='))?.split('=')[1] ?? '1200');

    // Custom state and report prefixes for parallel runs
    const stateFileFlag = args.find(a => a.startsWith('--state-file='))?.split('=')[1];
    const STATE_FILE  = stateFileFlag ? path.resolve(stateFileFlag) : path.join(__dirname, '..', 'logs', `ai-audit-state-${provider}.json`);
    
    const reportPrefixFlag = args.find(a => a.startsWith('--report-prefix='))?.split('=')[1] ?? `ai-mapping-audit-${provider}`;
    const REPORT_BASE = path.join(__dirname, '..', 'logs', `${reportPrefixFlag}-${new Date().toISOString().replace(/[:.]/g, '-')}`);

    const baseUrl = provider === 'ollama' ? OLLAMA_BASE_URL : OPENROUTER_BASE_URL;
    const apiKey = provider === 'ollama' ? 'ollama' : OPENROUTER_API_KEY;

    if (provider === 'openrouter' && !OPENROUTER_API_KEY && !isDryRun) {
    ...
    let subset = usdaOnly ? rawRows.slice(-1590) : rawRows;
    if (skipRows > 0) subset = subset.slice(skipRows);
    if (limitRows > 0) subset = subset.slice(0, limitRows);
    console.log(`   Total rows: ${rawRows.length}, auditing: ${subset.length} (skip: ${skipRows}, limit: ${limitRows})`);
    ...
    for (const [batchIdx, batch] of batches.entries()) {
        ...
        try {
            verdicts = await callGemini(batch, model, baseUrl, apiKey);
        } catch (err) {
