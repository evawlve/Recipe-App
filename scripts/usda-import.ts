...
import { importUsdaGenerics } from '../src/ops/usda/importer';
import { fdcToUsdaRow } from '../src/ops/usda/normalize';
import fs from 'fs';
...
async function convertToUsdaRows(foods: any[]): Promise<any[]> {
  ...
  for (const food of foods) {
    const usdaRow = fdcToUsdaRow(food);
    if (usdaRow) {
...
async function main() {
  ...
  try {
    ...
    const result = await importUsdaGenerics(usdaRows, options);
