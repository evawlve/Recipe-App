...
| Table | Purpose |
|-------|---------|
| `FoodMapping` | Maps normalized ingredient name → food targets (USDA FDC or OFF barcode) |
...
| Table | Purpose |
|-------|---------|
| `FdcFood` | Seeded USDA FoodData Central entries |
| `FdcServing` | Serving sizes for FDC foods (uses integer `fdcId`) |
| `OffFood` | Seeded Open Food Facts products (uses barcode string as `@id`) |
| `OffServing` | Serving sizes for OFF products (uses barcode string `barcode`) |

> **Important**: FDC foods use integer `fdcId`. Open Food Facts (OFF) products use barcode string `barcode` as key. Never mix these.
...
| `IngredientCleanupPattern` | Rules for prep/size phrase cleanup (referenced by learned-patterns.ts) |
...
3. If found → Use cached offBarcode/fdcId, proceed to serving selection
...
2. Query seeded `FdcFood` table (parallel search/scoring)
3. Query seeded `OffFood` table (parallel search/scoring)
4. Merge candidates from both local sources
...
3. Score each candidate:
   - Positional relevance from local db queries
   - Name similarity/overlap
   - Modifier matching (lowfat, organic)
   - Source preference (FDC preferred for produce, OFF for branded items)
...
1. Retrieve winner details from local `FdcFood` or `OffFood`
2. Resolve servings (FdcServing or OffServing)
3. AI Backfill missing servings (count/volume/weight) if missing from seeded tables
