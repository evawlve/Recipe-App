...
export async function gatherCandidates(
    ...
    // Live/Mock FatSecret client search (for tests and fallbacks)
    if (options.client && typeof options.client.searchFoodsV4 === 'function') {
        searchPromises.push((async () => {
            try {
                const res = await options.client.searchFoodsV4(primaryQuery, maxPerSource);
                const foods = Array.isArray(res) ? res : (res?.foods || []);
                return foods.map((food: any, position: number) => ({
                    id: food.id,
                    source: 'ai_generated', // Map to ai_generated to keep original ID for mock client lookup
                    name: food.name,
                    brandName: food.brandName || null,
                    score: 0.9 - (position * 0.02),
                    rawData: food,
                }));
            } catch (err) {
                logger.warn('gather.fatsecret.search_failed', { query: primaryQuery, error: (err as Error).message });
                return [];
            }
        })());
    }
