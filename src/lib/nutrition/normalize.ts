...
export const DEBUG = process.env.NUTRITION_DEBUG === "1";
...
export function servingToGrams(raw: RawFood): number | null {
  ...
  function pick(table:any): number | null {
    ...
    if (cat.includes("starch")) return table.starch ?? null;
    return null;
  }
...
export function sanitizePer100g(n: Per100g, raw?: RawFood): { n: Per100g; sanitized: boolean } {
...
export function extractCategoryHint(name: string, brand?: string): string | null {
  ...
  if (/oil|olive|avocado|canola|sunflower|sesame|vegetable oil|palm oil|coconut oil/i.test(text)) return "oil";
  ...
  if (/starch|corn starch|potato starch/i.test(text)) return "starch";
