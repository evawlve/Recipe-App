 ...
 */
export function gramsForMeasure(unit: string, categoryHint: string): number | null {
  // Normalize unit synonyms
  const normalizedUnit = normalizeUnit(unit);
  const cat = categoryHint.toLowerCase();
  
  const pick = (m: Record<string, number>) =>
    cat.includes('oil') ? m.oil :
    cat.includes('liquid') ? m.water :
    cat.includes('flour') || cat.includes('starch') ? m.flour : null;

  if (normalizedUnit === 'tsp') return pick({ oil: 4.5, water: 4.9, flour: 2.5 }) ?? null;
  if (normalizedUnit === 'tbsp') return pick({ oil: 13.6, water: 14.7, flour: 8.0 }) ?? null;
  if (normalizedUnit === 'cup') return (
    cat.includes('oil') ? 216 :
  ...
  if (normalizedUnit === 'g' || normalizedUnit === 'gram' || normalizedUnit === 'grams') return 1;
  if (normalizedUnit === 'ml') return 1; // water-like default
  return null;
}
 ...
 */
export function calculatePerServingCalories(
...
export function extractCategoryHint(name: string, brand?: string): string | null {
  ...
  if (/oil|olive|canola|avocado|sunflower|sesame|vegetable oil|palm oil|coconut oil/i.test(text)) return "oil";
  if (/water|milk|broth|stock|liquid/i.test(text)) return "liquid";
  if (/flour|starch|corn starch|almond flour|coconut flour|corn grain/i.test(text)) return "flour";
