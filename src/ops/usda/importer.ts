...
import { normalizeUsdaRowToPer100g } from './normalize';
import { canonicalizeName, macroFingerprint, generateAliases } from './dedupe';
import { UsdaRow, ImportResult, ImportOptions } from './types';

/**
 * Import USDA generics from bulk data
 */
export async function importUsdaGenerics(
  rows: UsdaRow[], 
...
async function processBatch(
  ...
  for (const row of rows) {
    try {
      // Normalize the row
      const per100 = normalizeUsdaRowToPer100g(row);
      if (!per100) {
        result.skipped++;
        continue;
      }

      const name = canonicalizeName(per100.name);
      if (!name) {
        result.skipped++;
        continue;
      }
      ...
      if (options.skipDuplicates) {
        ...
        const dupe = await prisma.food.findFirst({
          where: {
            source: 'usda',
            OR: [
      ...
      if (!options.dryRun) {
        ...
        const aliases = generateAliases(per100.name);
 ...
 * Add auto-units based on category
 */
async function addAutoUnits(foodId: string, categoryId?: string): Promise<void> {
  if (!categoryId) return;

  const unitMappings: Record<string, Array<{ label: string; grams: number }>> = {
    oil: [
 ...
 */
export async function importFromFdcApi(
  queries: string[], 
  options: ImportOptions = {}
): Promise<ImportResult> {
