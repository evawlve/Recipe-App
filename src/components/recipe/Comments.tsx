...
type CommentItem = { id:string; body:string; createdAt:string; user:{ id:string; name:string|null } };
...
export default function Comments({ recipeId, initial, canPost, currentUserId, recipeAuthorId }:{ recipeId:string; initial: CommentItem[]; canPost: boolean; currentUserId?: string|null; recipeAuthorId?: string }) {
	...
	async function submit() {
		...
		setErr("");
		const optimistic: CommentItem = { id: `tmp_${Date.now()}`, body, createdAt: new Date().toISOString(), user: { id: "me", name: "You" } };
