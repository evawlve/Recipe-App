...
exports.autoMapIngredients = autoMapIngredients;
 ...
 */
async function autoMapIngredients(recipeId) {
    logger_1.logger.info('autoMap:start', { recipeId, v2: flags_1.FOOD_MAPPING_V2 });
    ...
    for (const ingredient of ingredients) {
        ...
        const matchingFoods = await db_1.prisma.food.findMany({
            where: {
                OR: [
        ...
        if (bestMatch) {
            ...
            try {
                if (flags_1.FOOD_MAPPING_V2) {
                    await db_1.prisma.ingredientFoodMap.create({
                        data: {
                    ...
                    const existingMapping = await db_1.prisma.ingredientFoodMap.findFirst({
                        where: {
                            ingredientId: ingredient.id,
                            foodId: bestMatch.id,
                        },
                    ...
                    if (existingMapping) {
                        ...
                        await db_1.prisma.ingredientFoodMap.update({
                ...
                logger_1.logger.info('autoMap:mapped', { ingredientId: ingredient.id, foodId: bestMatch.id, confidence });
                mappedCount++;
            }
            catch (err) {
                logger_1.logger.warn('autoMap:error-map', { ingredientId: ingredient.id, err: err.message });
            }
        }
    }
    logger_1.logger.info('autoMap:done', { recipeId, mappedCount });
    return mappedCount;
}
/**
 * Find the best matching food for an ingredient
 */
function findBestMatch(ingredientName, foods) {
    if (foods.length === 0)
        return null;
    const ingredientLower = ingredientName.toLowerCase();
    // Exact match (case-insensitive)
    const exactMatch = foods.find(food => food.name.toLowerCase() === ingredientLower);
    if (exactMatch)
        return exactMatch;
    // Handle common variations and synonyms
    const variations = getIngredientVariations(ingredientName);
    for (const variation of variations) {
        const match = foods.find(food => food.name.toLowerCase().includes(variation.toLowerCase()) ||
            variation.toLowerCase().includes(food.name.toLowerCase()));
        if (match)
            return match;
    }
    // Partial match (contains)
    const partialMatch = foods.find(food => food.name.toLowerCase().includes(ingredientLower) ||
 ...
 */
function getIngredientVariations(name) {
    const variations = [];
 ...
 */
function calculateConfidence(ingredientName, foodName) {
    ...
    if (food.includes(ingredient) || ingredient.includes(food))
        return 0.9;
    // Partial match
    const commonWords = ingredient.split(' ').filter(word => food.includes(word) && word.length > 2);
