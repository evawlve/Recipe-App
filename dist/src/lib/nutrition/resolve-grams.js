...
exports.resolveGramsFromParsed = resolveGramsFromParsed;
/**
 * Resolve grams from parsed ingredient using serving options
 * Handles count units by matching serving option labels
 */
function resolveGramsFromParsed(parsed, servingOptions) {
    if (!parsed || servingOptions.length === 0)
        return null;
    ...
    if (parsed.unit) {
        ...
        const matchingOption = servingOptions.find(option => option.label.toLowerCase().includes(parsed.unit.toLowerCase()));
        if (matchingOption) {
