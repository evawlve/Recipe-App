        ...
        if (!error && data.user) {
            ...
            try {
                ...
                if (needsProfileSetup) {
                    ...
                    return server_1.NextResponse.redirect(`${origin}/signup?verified=true&email=${encodeURIComponent(data.user.email || '')}${googleParam}`);
                }
                else {
                    ...
                    return server_1.NextResponse.redirect(`${origin}${finalRedirectTo}`);
                }
            }
            catch (dbError) {
            ...
            return server_1.NextResponse.redirect(`${origin}/signin?error=auth_callback_error&message=${encodeURIComponent(error?.message || 'Authentication failed')}`);
        }
    }
    // If there's an error or no code, redirect to signin
    return server_1.NextResponse.redirect(`${origin}/signin?error=auth_callback_error&message=${encodeURIComponent('No verification code provided')}`);
}
