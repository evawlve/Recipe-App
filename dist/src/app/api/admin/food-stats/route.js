...
async function GET(req) {
    try {
        ...
        const totalFoods = await db_1.prisma.food.count();
        // Get counts per source
        const sourceStats = await db_1.prisma.food.groupBy({
            by: ['source'],
            _count: {
                id: true
            }
        ...
        const categoryStats = await db_1.prisma.food.groupBy({
            by: ['categoryId'],
            _count: {
                id: true
            },
        ...
        const recentAdditions = await db_1.prisma.food.groupBy({
            by: ['source'],
            _count: {
                id: true
            },
        ...
        let totalBarcodes = 0;
        try {
            totalUnits = await db_1.prisma.foodUnit.count();
        }
        catch (e) {
            console.log('FoodUnit error:', e);
        }
        try {
            totalAliases = await db_1.prisma.foodAlias.count();
        }
        catch (e) {
            console.log('FoodAlias error:', e);
        }
        try {
            totalBarcodes = await db_1.prisma.barcode.count();
        }
        catch (e) {
            console.log('Barcode error:', e);
        }
        // Get top categories
        const topCategories = await db_1.prisma.food.groupBy({
            by: ['categoryId'],
            _count: {
                id: true
            },
        ...
        const verificationStats = await db_1.prisma.food.groupBy({
            by: ['verification'],
            _count: {
                id: true
            }
