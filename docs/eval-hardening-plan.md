...
- **Deliberately skipped**: adding bare `c4`/`bang` to the brand lexicon (n-brand-05/06 stay knownIssue) — those unigrams would flip `isBranded` on lines like "bang bang shrimp" for zero eval gain since the SKUs are ingest-blocked anyway. *(Conclusion stands; the rationale needs one caveat since PR #230 — a non-decisive static hit can now be downgraded by the model, so a lexicon false positive is no longer unconditionally sticky. It is still sticky wherever `hasDecisiveBrandContext()` holds, which "bang bang shrimp" does not, so this case is unchanged.)*
